...
public class CaptureActivity extends Activity implements Callback ,OnClickListener{
	...
	private EditText mEditText;//生成二维码输入框
	...
	public  TextView tvCenterHint;//屏幕提示
	...
	//绘制的扫描框大小（绘制显示的 < 真正的）
	public static int sDrawWidth;
	public static int sDrawHeight;
	//真正扫描框大小
	public static int sRealWidth;
	public static int sRealHeight;
	public static boolean sIsReset = false;//是否重新设置扫描框大小
	 ...
	 * @param realHeight
	 */
	public void setScanRectWidthHeight(int drawWidth,int drawHeight,int realWidth,int realHeight,boolean isReset){
		sDrawWidth = drawWidth;
		sDrawHeight = drawHeight;
		sRealHeight = realHeight;
		sRealWidth = realWidth;
		sIsReset = isReset;
	}
...
//			e.printStackTrace();
	...
	private void initOnResume(){
		...
		Log.i("TAG", "onResume--hasSurface:"+hasSurface);
		if (hasSurface) {
...
//		goPageURL(obj.getText());
	...
	public void onClick(View v) {
		if(v== mSwitchLight){//开关闪光灯
			...
			if(result){//闪光灯已经打开
				mLightText.setText("打开");
				mLightText.setTextColor(Color.parseColor("#f4ea2a"));
				mSwitchLight.setSelected(true);
			}else{
				mLightText.setText("关闭");
				mLightText.setTextColor(Color.parseColor("#cdcdcd"));
				mSwitchLight.setSelected(false);
			}
		}else if(v == m2DCode){//生成我的二维码
			mEditText = new EditText(CaptureActivity.this);
			//弹出输入框
				...
				public void onClick(DialogInterface dialog, int which) {
					try {
						if(mEditText.getText().toString() !=null && !mEditText.getText().toString().equals("")){
							...
							//生成我的二维码
							myBitmap = Create2DCode(mEditText.getText().toString().trim());
...
//			Toast.makeText(this, "result-->"+strResult, 0).show();
	...
	private void share(String content, Uri uri){
		...
		if(uri!=null){
			...
			shareIntent.setType("image/*"); //图片
//	        shareIntent.setType("video/*");//视频
//	        shareIntent.setType("audio/*");//音频
		...
		//自定义选择框的标题
//	    startActivity(Intent.createChooser(shareIntent, "我的分享"));
		//系统默认标题
