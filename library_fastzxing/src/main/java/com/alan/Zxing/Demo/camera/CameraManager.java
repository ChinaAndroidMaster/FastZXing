...
//      }
      if(CaptureActivity.sIsReset){//如果自定义了大小
        width = CaptureActivity.sDrawWidth;
        height = CaptureActivity.sDrawHeight;
      }
   ...
   * @return
   */
  public Rect getRealScanFramingRect() {
    //得到屏幕信息（宽高）
    Point screenResolution = configManager.getScreenResolution();
    if (framingRect == null) {
      if (camera == null) {
...
//      }
//      int height = width ;//正方形框
      int height = screenResolution.y;//和屏幕大小一样
//      int height = width/4 ;//矩形框，条形码
      //限制最大最小值
//      if (height < MIN_FRAME_HEIGHT) {
//        height = MIN_FRAME_HEIGHT;
...
//      }
      if(CaptureActivity.sIsReset){//如果自定义了大小
        width = CaptureActivity.sRealWidth;
        height = CaptureActivity.sRealHeight;
      }
      //确定左上角的位置(也就是确定扫描框的位置)
      int leftOffset = (screenResolution.x - width) / 2;
      int topOffset = (screenResolution.y - height - 20) / 2;
